...
fn add_new_nn() {
    ...
    let nn_data = NeuronalNetworkData {
        id: USERS[1].into(),
...
fn add_new_nn_fail() {
    ...
    let mut expected_log = Log::builder()
        .dest(USERS[2])
        .payload(ContractEvent::NotTheOwner);
...
fn subscribe_user_free() {
    ...
    // Subscribing user to free neuronal networks
    ...
    res = main_contract.send(
        USERS[2],
        ContractAction::Subscribe(SubscriptionType::Free)
    );
...
fn subscribe_user_free_fail() {
    ...
    // must fail, user is already subscribed
